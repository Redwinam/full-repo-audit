...
- Check: `status=pending|in_progress|stale|reviewed|unreviewable|not_applicable`; `evidence` is an array of specific references/observations. `reviewed` needs substantive evidence; `not_applicable` needs evidence and reason; `unreviewable` needs evidence, reason, impact and unblock action. Add domain-specific checks freely; the defaults in review-standards.md remain mandatory.
